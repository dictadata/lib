...
/**
 * Wrapper for Node.js HTTP, HTTPS and HTTP/2 modules.
...
function httpRequest(url, request, data) {
  ...
  if (typeof url === "string") {
    ...
    throw new libError(`Invalid url ${url}`);
  }
        ...
        res.on('end', () => {
          ...
          if (encoding) {
            if (encoding === 'gzip')
              ...
              throw new libError(`unknown content-encoding: ${encoding}`);
          }
          else {
    ...
    if (request.responseType === 'stream') {
      ...
      req.on('response', (res) => {
        if (res.statusCode !== 200) {
          logger.warn(`httpRequest res.statusCode ${res.statusCode}`);
          res.destroy();
          reject({ statusCode: res.statusCode, statusMessage: res.statusMessage, headers: res.headers });
        }
