...
const contentTypeIsJSON = require('../utils/contentTypeIsJSON');
...
async function httpQueries(queryFile, queryName) {
  ...
  let outputDir = path.dirname(queryFile).replace("/test/", "/test/_data/output/") + queryName + "/" + httpQueries.name + "/";
...
async function submitQuery(request, expected, outputFile) {
  ...
  try {
    ...
    let results;
    let isJSON = contentTypeIsJSON(response.headers[ "content-type" ]);
    if (isJSON) {
